...
vignette: >
  %\VignetteIndexEntry{How to use package athmc: automatically-tuned, hamiltonian Monte Carlo}
...
## Running ATHMC
```{r}
niter <- 10 # length of each constructed chain
truepower <- 2 # the polynomial degree of U is 2. That is, the target is a mixture of Gaussians.
## construct a chain
set.seed(829857)
...
If instead `maxEta_tuning_method="ellipsoidal"`, then an ellipsoidal search scope is used: $\max_{k\in 0\col K} \sum_{j=1}^d \{x_j\{k\} - x_j^0\}^2 / s_j^2 > d$, where $x_j\{k\}$ is the $j$-th coordinate of the $k$-th intermediate point along the numerically simulated path, $x^0$ is the `center_point`, and $s_j$ is the `search_scale`.
...
As you can see from the plot below, ATHMC can construct chains that facilitate transitions between isolated modes for high-dimensional ($d=10000$), strongly-multimodal distributions.

```{r}
plot(chain$sumstat_MCMC)
```
